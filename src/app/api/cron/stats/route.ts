...
export async function GET(req: NextRequest) {
  ...
  try {
    // Query all sessions from the last 24 hours
    ...
    const avgMessages = total
      ? Math.round(
          (sessions!.reduce((sum, s) => sum + (s.message_count ?? 0), 0) / total) * 10
        ) / 10
      : 0;
    ...
    const leads =
      sessions
        ...
        .map(
          (s) =>
            `<tr><td>${s.user_name ?? "—"}</td><td><a href="mailto:${s.user_email}">${s.user_email}</a></td><td>${s.message_count}</td></tr>`
        )
        .join("") ?? "";
    ...
    const html = `
<h2>Ela by Edualist — Günlük İstatistik Raporu</h2>
...
`
    : "<p>Son 24 saatte sohbet yapılmadı.</p>"
}
...
`
    ...
    const { error: mailError } = await getResend().emails.send({
      from: "Ela <noreply@edubot.com.tr>",
      to: "ceo@edualist.com",
      subject: `Ela Günlük Rapor — ${date}`,
      html,
    });
    ...
    console.error("Stats cron error:", e);
